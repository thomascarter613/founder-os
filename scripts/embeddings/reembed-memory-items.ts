...
function getItemsToReembed(
  ...
  const whereClause = reembedAll
    ? "true"
    : `
        me.id is null
        or me.embedding_provider <> ${sqlString(provider)}
        or me.embedding_model <> ${sqlString(model)}
        or me.embedding_dimensions <> ${dimensions}
      `;
...
async function main(): Promise<void> {
  ...
  const limit = Number(process.env.REEMBED_LIMIT ?? "100");
  ...
  const reembedAll = process.env.REEMBED_ALL === "true";
  const schemaMode = detectSchemaMode(databaseUrl);
  const items = getItemsToReembed(databaseUrl, limit, provider, model, dimensions, reembedAll);

  if (items.length === 0) {
    console.log("No memory items require re-embedding.");
    return;
  }
  ...
  for (const item of items) {
    ...
    console.log(`Re-embedded memory item ${item.id} using ${provider}/${model}.`);
  }
}
